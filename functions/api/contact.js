...
function buildEmail({ name, email, message }, toEmail) {
  ...
  return { subject, text, html, to: toEmail };
}
...
export async function onRequestPost({ request, env }) {
  ...
  if (!toEmail || !fromEmail || !env.EMAIL_API_KEY) {
    return jsonResponse({ ok: false, error: 'Contact email service is not configured.' }, 503);
  }

  const emailData = {
    ...buildEmail(validation, toEmail),
